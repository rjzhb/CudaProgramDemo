...
__global__ void what_is_my_id_2d_A(
    ...
    thread[thread_idx] = blockIdx.x;
    calc_thread[thread_idx] = threadIdx.x;
...
int main() {
    ...
    int kernel = 0;
    scanf_s("input kernel%d\n", kernel);

    switch (kernel) {
        case 0: {
            what_is_my_id_2d_A<<<blocks_rect, threads_rect>>>(gpu_block_x, gpu_block_y, gpu_thread, gpu_calc_thread,
                                                              gpu_xthread, gpu_ythread, gpu_grid_dimx,
                                                              gpu_block_dimx, gpu_grid_dimy, gpu_block_dimy);
            break;
        }
        case 1: {
            what_is_my_id_2d_A<<<blocks_square, threads_square>>>(gpu_block_x, gpu_block_y, gpu_thread,
                                                                  gpu_calc_thread,
                                                                  gpu_xthread, gpu_ythread, gpu_grid_dimx,
                                                                  gpu_block_dimx, gpu_grid_dimy, gpu_block_dimy);
            break;
